...
def line x, y, x1, y1
  '<line x1="'+x.to_s+'" y1="'+y.to_s+'" x2="'+x1.to_s+'" y2="'+y1.to_s+'" style="stroke:rgb(0,0,0);stroke-width:3" />'
end
			...
			value.each do |new_key,new_value|
				new_key.each do |newer_key,classes|
					if (newer_key=='node_name')
						...
						f.write(line(def_pos_x,def_pos_y,def_pos_x-temp_end,def_pos_y-200))
						...
						classes.each do |newest_key,newest_value|
							newest_key.each do |name,name_value|
								...
								puts "  #{name_value}"
								f.write(line(def_pos_x-200,def_pos_y,def_pos_x-temp_x,def_pos_y+160))
