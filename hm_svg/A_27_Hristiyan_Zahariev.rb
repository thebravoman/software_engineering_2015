...
File.open("A_27_Hristiyan_Zahariev.svg", "w") do |f|
    ...
    if(key1 != node_name)
      value1.each do |key2, value2|
        key2.each do |key3, value3|
		  ...
		  f.write(text_colored (leafs_x + textleafs_x - 93), 550, value5) #<-----Names
		  textleafs_x+=100
		  f.write(lines (leafs_x + textleafs_x - 150), 500, (class_x + 100), 270) 
                end			
            end
            class_x += 500
          end	
  ...
  f.write('<text x="350" y="700" fill="red" font-size="70">Cherry tree Kappa</text>')
  f.write('</svg>')
end
