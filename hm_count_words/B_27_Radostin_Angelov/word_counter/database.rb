module WordCounter
    ...
    class Database
        ...
        def save(identifier, words_count, marks_count)
            ...
            #word_counts_key is a string, which i make a string
            word_counts_key = Digest::SHA256.digest(content).to_s
            identifier = identifier.to_s

            #here i insert word_counts_key and just in case i surround it with '
            #so it is a string no matter the case... it is a string on so many levels...
            #ruby knows it is not a string, so it takes only the first part before the space, and shits bricks
=begin
            @db.execute(
                "INSERT INTO statistics(source_name, hash)
                VALUES (\'#{identifier}\', \'#{word_counts_key}\') ")

            curr_row = @db.execute("SELECT id FROM statistics
                WHERE hash = #{word_counts_key}")
=end
            @db.execute(
                "INSERT INTO statistics(source_name, hash)
